...
def find_maximum_distinct_elements(nums, k):
  ...
  # Add all elements with freq > 1 to minheap
  ...
  # Pop out element with lowest frequency 
  # Check if removing all occurances of this element but 1 results in k removals
  while minheap and k > 0:
      freq, num = heappop(minheap)
      k = k - (freq - 1)
      if k >= 0:
          distinct += 1
  # All elements have been taken out of the minheap
  # Remove remaining elements to get count of distinct elements
  if k > 0:
      distinct -= k
...
# Since the heap can have a maximum of ‘N/2’ numbers, therefore, extracting an element from the heap will take O(logN)
